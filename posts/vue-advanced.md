...
最直接的感受：**写 Pinia 就像写一个 composable，几乎没有学习成本。**
...
Setup store 模式（上面这种）是我现在唯一用的方式。它跟 Composition API 完全同构——从组件里搬出来就是 store，反之亦然。
...
Vue 3 的 Composition API 依赖组件实例的 `setup` 上下文。在普通函数、事件回调里调用 `useStore` 之类的 composable，会报 `inject() can only be used inside setup()`。解决方案：要么把 composable 的调用放在 setup 内，要么显式传参。
...
### 不要把逻辑全塞在一个 composable 里

一个 composable 超过 100 行就要考虑拆分了。按照「数据 → 计算 → 操作」三段式拆分，参照 Pinia 的设计哲学——一个 composable 只做一种逻辑的封装。

## 总结

Vue 3 的进阶用法，核心其实就是三件事：

1. **Pinia** — 替代 Vuex，Setup Store 模式跟 Composition API 无缝衔接
2. **provide/inject** — 好好用起来，比 props drilling 优雅得多，但要小心响应式陷阱
3. **Composition API 自律** — 框架给你了自由，但别滥用
